...
// Widgets
static const float          DRAGDROP_HOLD_TO_OPEN_TIMER = 0.70f;    // TimeManager for drag-hold to activate items accepting the ImGuiButtonFlags_PressedOnDragDropHold button behavior.
...
// CreateBuffer text input in place of another active widget (e.g. used when doing a CTRL+Click on drag/slider widgets)
...
bool ImGui::CollapsingHeader(const char* label, bool* p_visible, ImGuiTreeNodeFlags flags)
{
    ...
    if (p_visible != NULL)
    {
        // CreateBuffer a small overlapping close button
