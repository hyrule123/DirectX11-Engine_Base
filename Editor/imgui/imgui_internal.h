...
{
    ImGuiWindow*        Window;         // Setting,Move    // Best candidate window (result->ItemWindow->RootWindowForNav == request->Window)
    ImGuiID             ID;             // Setting,Move    // Best candidate item ID
    ImGuiID             FocusScopeId;   // Setting,Move    // Best candidate focus scope ID
    ImRect              RectRel;        // Setting,Move    // Best candidate bounding box in window relative space
...
struct ImGuiContext
{
    ...
    // Navigation: Setting & Move Requests
    bool                    NavAnyRequest;                      // ~~ NavMoveRequest || NavInitRequest this is to perform early out in ItemAdd()
    bool                    NavInitRequest;                     // Setting request for appearing window to select first item
    bool                    NavInitRequestFromMove;
    ImGuiID                 NavInitResultId;                    // Setting request result (first item of the window, or one for which SetItemDefaultFocus() was called)
    ImRect                  NavInitResultRectRel;               // Setting request result rectangle (relative to parent window)
...
//#endif
    ...
    IMGUI_API ImGuiID       DockBuilderSplitNode(ImGuiID node_id, ImGuiDir split_dir, float size_ratio_for_node_at_dir, ImGuiID* out_id_at_dir, ImGuiID* out_id_at_opposite_dir); // CreateBuffer 2 child nodes in this parent node.
