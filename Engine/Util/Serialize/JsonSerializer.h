...
public:
	...
	static Json::Value NewJson(const T& _val);

	template <NotPointerTypes T>
	static T GetValueFromJson(const Json::Value& _jVal);
...
inline void JsonSerializer::Write(const std::string& _strKey, const std::vector<T>& _vector)
{
	...
	{
		vectorValue.append(NewJson(_vector[i]));
	}
...
template<NotPointerTypes T>
inline Json::Value JsonSerializer::NewJson(const T& _val)
{
	Json::Value retJval;
...
template<NotPointerTypes T>
inline T JsonSerializer::GetValueFromJson(const Json::Value& _jVal)
{
	T retVal{};
