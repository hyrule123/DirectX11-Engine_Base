...
namespace ehw
{
	...
	HRESULT StructBuffer::Create(size_t _uElemStride, size_t _uElemCapacity, const void* _pInitialData, size_t _uElemCount)
	{
		...
		case eStructBufferType::READ_ONLY:
		{
			...
			if (FAILED(GPUManager::Device()->CreateBuffer(&mBufferDesc, pData, mBuffer.ReleaseAndGetAddressOf())))
			{
		...
		{
			{
				...
				if (SUCCEEDED(pContext->Map(mBuffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &Data)))
				{
		...
		{
			{
				...
				if (SUCCEEDED(pContext->Map(mStagingBuffer.Get(), 0, D3D11_MAP_WRITE, 0, &Data)))
				{
					...
					pContext->CopyResource(mBuffer.Get(), mStagingBuffer.Get());
				}
				break;
			}
	...
	void StructBuffer::GetData(void* _pDest, size_t _uDestByteCapacity)
	{
		...
		case eStructBufferType::READ_ONLY:
		{
			...
			//		D3D11_MAPPED_SUBRESOURCE Data = {};
			//
			//		pContext->Map(mBuffer.Get(), 0, D3D11_MAP_READ, 0, &Data);
			//
		...
		case eStructBufferType::READ_WRITE:
		{
			...
			pContext->CopyResource(mStagingBuffer.Get(), mBuffer.Get());
	...
	void StructBuffer::BindDataUAV(int _UAVSlot)
	{
		...
		uint Offset = -1;
		GPUManager::Context()->CSSetUnorderedAccessViews(_UAVSlot, 1, mUAV.GetAddressOf(), &Offset);
	}
	...
	bool StructBuffer::CreateStagingBuffer()
	{
		...
		bool bResult = SUCCEEDED(GPUManager::Device()->CreateBuffer(&Desc, nullptr, mStagingBuffer.ReleaseAndGetAddressOf()));
	...
	bool StructBuffer::CreateSRV()
	{
		...
		bool bResult = SUCCEEDED(GPUManager::Device()->CreateShaderResourceView(mBuffer.Get(), &SRVDesc, mSRV.ReleaseAndGetAddressOf()));
	...
	void StructBuffer::UnBindData()
	{
		...
		case ehw::eBufferViewType::UAV:
		{
			...
			GPUManager::Context()->CSSetUnorderedAccessViews(mCurBoundRegister, 1, &pUAV, &v2_Offset);
			break;
		}
