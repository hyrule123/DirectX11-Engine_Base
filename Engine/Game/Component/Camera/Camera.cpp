...
namespace core
{
	Camera::Camera()
		...
		, m_b_culling(true)
		, m_aspect_ratio(1.0f)
		, m_near_distance(1.0f)
		, m_far_distance(50000.f)
		, m_scale(1.0f)
	...
	void Camera::on_enable()
	{
		...
		RenderManager::get_inst().sceneRenderAgent().register_camera(ths);
	}

	void Camera::on_disable()
	{
		s_ptr<Camera> ths = std::static_pointer_cast<Camera>(shared_from_this());
		RenderManager::get_inst().sceneRenderAgent().unregister_camera(ths);
	}

	void Camera::render_gameobjects(const tRenderQueue& _render_queue)
	{
		...
		for (const s_ptr<GameObject>& obj : _render_queue.objects_to_render) {
			if (true == m_layer_masks[obj->get_layer()]) {
	...
	void Camera::render_lights_3D(eLightType _light_type, const std::vector<w_ptr<Light3D>>& _lights)
	{
		m_transform_data.clear();
		...
		//나중에 컬링도 진행할것.
		for (const w_ptr<Light3D>& light : _lights) {
			s_ptr<Light3D> l = light.lock();

			s_ptr<GameObject> obj = l->get_owner();

			//레이어 마스크와 일치하는지 확인(bit mask)
			if (obj && true == m_layer_masks[obj->get_layer()]) {
				//transform 데이터 넣고
				tTransform trdata{};
				trdata.World = obj->get_component<Transform>()->get_world_matrix();
				trdata.InverseWorld = trdata.World.Invert();
...
#pragma endregion
	...
	void Camera::SetCullEnable(bool _bCullingEnable)
	{
		m_culling_agent.reset();

		m_b_culling = _bCullingEnable;
		if (m_b_culling)
		{
			switch (m_projection_type)
			{
			case core::eProjectionType::Perspective:
				m_culling_agent = std::make_unique<CullingAgent_Perspective>();
				break;
			case core::eProjectionType::Orthographic:
				m_culling_agent = std::make_unique<CullingAgent_Orthographic>();
				break;
			default:
	...
	void Camera::CreateProjectionMatrix(uint resolution_X, uint resolution_Y)
	{
		m_culling_agent.reset();

		float width = (float)resolution_X * m_scale;
		float height = (float)resolution_Y * m_scale;
		...
		case eProjectionType::Orthographic:
			m_camera_matrices.projection = MATRIX::CreateOrthographicLH(width /*/ 100.0f*/, height /*/ 100.0f*/, m_near_distance, m_far_distance);
	...
	void Camera::TurnLayerMask(uint32 _layer, bool _enable)
	{
		m_layer_masks.set((uint32)_layer, _enable);
	}

	void Camera::bind_data_to_GPU()
	{
