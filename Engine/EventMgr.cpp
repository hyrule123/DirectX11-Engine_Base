...
namespace ehw
{
	std::vector<std::function<void()>> EventMgr::m_FrameEndEvents{};
	...
	void EventMgr::FrameEnd()
	{
		for (size_t i = 0; i < m_FrameEndEvents.size(); ++i)
		{
			m_FrameEndEvents[i]();
		}
		m_FrameEndEvents.clear();
	}
	...
	{
		ASSERT_DEBUG(eLayerType::None != _layer, "레이어를 지정하지 않았습니다.");
		ASSERT_DEBUG(_gameObj, "게임오브젝트가 설정되어 있지 않습니다.");
